...
class MulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        """TODO: Your code here"""
        return [
            mul_op(node.inputs[1], output_grad),
...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        if node.matmul_attr_trans_A and node.matmul_attr_trans_B:
            return input_vals[0].T @ input_vals[1].T
        if node.matmul_attr_trans_B:
            return input_vals[0] @ input_vals[1].T
        if node.matmul_attr_trans_A:
            return input_vals[0].T @ input_vals[1]
        else:
            return input_vals[0] @ input_vals[1]
...
class Executor:
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            input_vals = [node_to_val_map[node] for node in node.inputs]
            if node not in node_to_val_map:
                inter_val = node.op.compute(node, input_vals)
                node_to_val_map[node] = inter_val
...
def gradients(output_node, node_list):
    ...
    """TODO: Your code here"""
    # Create nodes for gradients

    for node in reverse_topo_order:
        # sum the contribution of all ancestors node
        sum_grad = sum_node_list(node_to_output_grads_list[node])
        node_to_output_grad[node] = sum_grad
        grad_nodes = node.op.gradient(node, sum_grad)
